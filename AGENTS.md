...
进度文档是后续 Codex 实例快速接手的唯一可靠入口，**禁止跳过更新**。
...
以下操作由**用户手动执行**，Codex 不要自动调用：

1. **依赖安装**（`uv pip install <pkg> -i https://pypi.tuna.tsinghua.edu.cn/simple`）—— Codex 只更新 `requirements.txt`，由用户执行安装命令。
2. **运行类命令**（`pytest`、`uvicorn`、联调脚本等）—— Codex 给出运行指令，由用户执行后把输出贴回。

Codex 可以读文件、写代码、写测试、写文档，但不要自作主张去 `pip install` 或启动长进程。
